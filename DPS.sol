...
contract cDPSContract is Manager{
	...
	address _secretary = 0x3C7Ba96F1F62081Fcb7AF0bFF4583d198ed660d6;   //Secretary Addr
    ...
    function SetupAll(address _sDEXAddr, address _sSecretaryAddr) public onlyManager{
...
contract DPSContract is cDPSContract, math{
	...
	uint LastTimestamp;         //The times of the last update.
	uint CalculationTimes;      //The times since the last update to the present.
	uint TotalContValue;        //The number of counts since the last distributed income to the present.
	uint UpdateTimes;           //Total GET pay out.
	uint TotalGETOut;           //Total GET pay out.
	uint DPSPayAmounts;         //DPS income from the last distribution..
	
    struct DepositInfo {
        uint RegTime;           //Time of user registration.
        uint timestamp;         //Time of the last update.
        uint ContributionValue; //Contribution value since the last distributed income to the present.
        uint UnclaimedIncome;   //Unclaimed DPS income.
    }
    ...
    function inqDPS() public view returns(uint[] memory){
		...
		uint[] memory returnint = new uint[](8);
		returnint[0] = RegFee;
		...
		returnint[6] = DPSPayAmounts;
		returnint[7] = getWhitelistLength();
